...
namespace AirPlane.WebUI.Infrastructure.Views
{
    public class DebugDataView : IView
    {
        public void Render(ViewContext viewContext, TextWriter writer)
        {
            ...
            {
                Write(writer, "Key: {0}, Values: {1}", key, viewContext.RouteData.Values[key]);
            }
            Write(writer, "---View Data ----");
            foreach (var key in viewContext.ViewData.Keys)
            {
                Write(writer, "Key: {0}, Values: {1}", key, viewContext.ViewData[key]);
            }
        }
